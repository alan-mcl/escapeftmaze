...
public class CharacterClassSelection extends JPanel implements ActionListener
{
	...
	/*-------------------------------------------------------------------------*/
	public void refreshCharacterClasses(Collection<CharacterClass> allowedCharacterclasss)
	{
		if (allowedCharacterclasss == null)
		{
			// select all
			for (JCheckBox cb : checkBoxes.values())
			{
		...
		for (CharacterClass cc : allowedCharacterclasss)
		{
			JCheckBox cb = checkBoxes.get(cc.getName());
			cb.setSelected(true);
		}
	}
