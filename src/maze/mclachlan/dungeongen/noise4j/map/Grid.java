package mclachlan.dungeongen.noise4j.map;

import java.util.Arrays;

import mclachlan.dungeongen.noise4j.array.Array2D;
...
public class Grid extends Array2D
{
	...
	/**
	 * @param grid   array that will internally used by the grid. Its size has to
	 *               be equal width multiplied by height.
	 * @param width  amount of columns.
	 * @param height amount of rows.
	 */
	...
	/**
	 * @return direct reference to the stored array. Use in extreme cases, try to
	 * use getters instead.
	 */
	 ...
	 * @param x     column index.
	 * @param y     row index.
	 * @param value will be added to the current value stored in the chosen
	 *              cell.
	 ...
	 * @param x     column index.
	 * @param y     row index.
	 * @param value will be subtracted from the current value stored in the
	 *              chosen cell.
	 ...
	 * @param x     column index.
	 * @param y     row index.
	 * @param value will be multiplied by the current value stored in the chosen
	 *              cell.
	 ...
	 * @param x   column index.
	 * @param y   row index.
	 * @param mod will be used to perform modulo operation on the current cell
	 *            value.
	 ...
	 *
	 * @param cellConsumer will consume each cell. If returns true, further
	 ...
	 */
	public void forEach(final CellConsumer cellConsumer, final int fromX,
		final int fromY)
	{
	 ...
	 * @param fromX        first cell column index. Min is 0.
	 * @param fromY        first cell row index. Min is 0.
	 * @param toX          last cell column index (excluded). Max is {@link
	 *                     #getWidth()}.
	 * @param toY          last cell row index (excluded). Max is {@link
	 *                     #getHeight()}.
	 */
	public void forEach(final CellConsumer cellConsumer, final int fromX,
		final int fromY, final int toX,
		final int toY)
	{
	 ...
	 * @param toIndex      actual array index (excluded); iterations ends with
	 *                     this value -1. Max is length of array.
	 */
	protected void iterate(final CellConsumer cellConsumer, final int fromIndex,
		final int toIndex)
	{
	...
	/**
	 * @param min all values lower than this value will be converted to this
	 *            value.
	 * @param max all values higher than this value will be converted to this
	 *            value.
	 * @return this, for chaining.
	 */
	...
	/**
	 * {@link #clone()} alternative with casted result. Cloning is not supported
	 * on GWT.
	 *
	...
	{
		/**
		 * Should be returned by {@link #consume(Grid, int, int, float)} method
		 * for code clarity.
		 */
		 ...
		 * @param value value stored in the current cell. Should match {@link
		 *              Grid#get(int, int)} method result invoked with passed
		 *              coordinates (x and y).
		 * @return if true and {@link CellConsumer} is used to iterate over the
		 * {@link Grid} using an iteration method like {@link
		 * Grid#forEach(CellConsumer)}, further iteration will be cancelled.
		 * @see #BREAK
		 * @see #CONTINUE
		 */
