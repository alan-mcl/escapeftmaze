package mclachlan.dungeongen.noise4j.map.generator;
...
{
	/**
	 * @param grid all (or most) of its cells will be affected, usually by adding
	 *             or subtracting their current cell value.
	 */
	void generate(Grid grid);

	/**
	 * @return current generation mode, deciding how values modify current grid's
	 * cells.
	 */
	...
	public static enum GenerationMode
	{
		...
		ADD
			{
				@Override
				public void modify(final Grid grid, final int x, final int y,
					final float value)
				{
					grid.add(x, y, value);
				}
