package mclachlan.dungeongen.noise4j.map.generator.room.dungeon;
...
import mclachlan.dungeongen.noise4j.array.Int2dArray;
...
/**
 * Generates a set of rooms with a maze-like system of corridors connecting
 * them. This particular implementation requires the map and rooms to have odd
 * sizes - if the passed map is not odd, last row and column might be filled
 * with corridors.
 * <p>
 * This algorithm fills the whole map. Even if the map was not empty before,
 * {@link #generate(Grid)} method will override the previous cell settings -
 * it's better to modify already generated dungeon rather than pass non-empty
 * grid to this generator.
 *
 * @author MJ
 */
...
public class DungeonGenerator extends AbstractRoomGenerator
{
	...
	/**
	 * Not thread-safe. Uses static generator instance. Since this method
	 * provides only basic settings, creating or obtaining an instance of the
	 * generator is generally preferred.
	 *
	...
	public static void generate(final Grid grid,
		final int roomGenerationAttempts)
	{
	 ...
	 * @return maximum amount of placed rooms. Used if {@link
	 * #roomGenerationAttempts} is not set.
	 */
	 ...
	 * @param attempts amount of attempts of placing rooms before the generator
	 *                 gives up.
	 */
	 ...
	 * @param point will start carving maze at this point. Stops when it reaches
	 *              a dead end.
	 */
	...
	/**
	 * @param grid  contains the point.
	 * @param point a point representing a part of a corridor. Should set its
	 *              value in the grid.
	 */
	 ...
	 */
	protected boolean isCarveable(final Point point, final Grid grid,
	 ...
	 */
	protected void joinRegions(final Grid grid)
	{ // DRAGON.
		nextRegion();
			...
			&& unjoined.size() > 1; )
		{
			...
			if (tempSet.size() <= 1)
			{ // All connector's regions point to the same region group...
	...
	/**
	 * Should change selected point's value. Note that connector might connect
	 * both two corridors and two rooms - spawning a door (for example) might not
	 * be always desired; check cell neighbors first.
	 *
	 * @param grid contains the point.
	 * @param x    column index.
	 * @param y    row index.
	...
	/**
	 * @param grid contains unconnected room and corridor regions.
	 * @return map of points that are neighbors to at least 2 different regions
	 * mapped to set of IDs of their neighbors.
	 */
	 ...
	 * @param connectorsToRegions map of possible connectors to the collection of
	 *                            regions that are their neighbors.
	 * @param x                   column index of possible connector.
	 * @param y                   row index of possible connector.
	 */
	protected void addConnector(final Grid grid,
		final Map<Point, Set<Integer>> connectorsToRegions, final int x,
		final int y)
	{
		if (isWall(grid, x, y))
		{
	 ...
	 * @param regionsIterator regions' iterator. Should have one value skipped
	 *                        (source).
	 * @param merged          contains mapping of regions to the IDs of their
	 *                        supergroups.
	 * @return regions marked as destinations.
	 * @see #joinRegions(Grid)
	 */
	protected Integer[] getDestinations(final Set<Integer> regions,
		final Iterator<Integer> regionsIterator,
	...
	/**
	 * @param grid    contains the cell.
	 * @param deadEnd a currently closed dead end that can possibly have a single
	 *                dead end neighbor.
	 ...
	 * @param x column index.
	 * @param y row index.
	 * @return true if selected cell is a corridor. Works only if the cell is not
	 * a wall. This check works if all rooms and corridors are already spawned.
	 */
	...
	/**
	 * @return amount of attempts of placing a new room during dungeon
	 * generation.
	 */
	...
	/**
	 * @param roomGenerationAttempts amount of attempts of placing a new room
	 *                               during dungeon generation. Changing this
	 *                               value allows to modify density of the
	 *                               rooms.
	 */
	...
	/**
	 * @return cells are considered walls if they are equal to or bigger than
	 * this value.
	 */
	...
	/**
	 * @param wallThreshold cells are considered walls if they are equal to or
	 *                      bigger than this value. Should be higher than {@link
	...
	/**
	 * @param floorThreshold cells are considered room floor if they are equal
	 *                       this value. Should be lower than {@link
	 *                       #getWallThreshold()}.
	 */
	...
	/**
	 * @param corridorThreshold cells are considered corridors if they are equal
	 *                          this value. Should be lower than {@link
	 *                          #getWallThreshold()}.
	 */
	...
	/**
	 * @return chance to wind the currently generated corridor in range of 0 to
	 * 1.
	 */
	...
	/**
	 * @param windingChance chance to wind the currently generated corridor in
	 *                      range of 0 to 1. Anything below (or equal to) 0
	 *                      results in winding the corridor only if cannot
	 *                      continue carving in the same direction (not the best
	 *                      idea to make a playable game). Anything above or
	 *                      equal to 1 results in completely random corridor
	 *                      carving, resulting in highly chaotic mazes. The
	 *                      higher the value, the more chaotic the result.
	 */
	...
	/**
	 * @return chance of a random carved cell between two regions (rooms and
	 * corridors) in range of 0 to 1.
	 */
	...
	/**
	 * @param randomConnectorChance chance of a random carved cell between two
	 *                              regions (rooms and corridors) in range of 0
	 *                              to 1. The higher this value, the more
	 *                              passages will appear between the rooms and
	 *                              different corridors. Setting this value to 0
	 *                              (or lower) will result in a "perfect"
	 *                              dungeon, which basically has only one way of
	 *                              solving (as in getting from 1 point to
	 *                              another, treating rooms as one cell). This
	 *                              might make the dungeon crawling linear, so
	 *                              this is usually not desired; however, using
	 *                              too high value might result in broken
	 *                              dungeons with a lot of unnecessary passages
	 *                              and doors.
	 */
	...
	/**
	 * @return amount of iterations performed to remove all dead ends in the
	 * corridors.
	 */
	...
	/**
	 * @param deadEndRemovalIterations amount of iterations performed to remove
	 *                                 all dead ends in the corridors. Each
	 *                                 iteration removes only the last dead end's
	 *                                 tile, not the whole branch. This should
	 *                                 not cause significant performance
	 *                                 penalties, but if you do want to leave
	 *                                 some dead ends in the final dungeon or
	 *                                 need the generation to be blazing fast,
	 *                                 you can limit this value. Note that also
	 *                                 generating more rooms - {@link
	 *                                 #setRoomGenerationAttempts(int)}, creating
	 *                                 more passages - {@link #setRandomConnectorChance(float)}
	 *                                 or making the dungeon more random - {@link
	 *                                 #setWindingChance(float)} - might decrease
	 *                                 the amount of dead ends (or overall
	 *                                 corridors amount).
	 */
	 ...
	 *
	 * @author MJ
	 */ // Avoids extra dependencies and classes not available on Android/GWT.
	protected static class Point
	{
		 ...
		 * @return true if the passed point is a direct (non-diagonal) neighbor of
		 * this point.
		 */
	...
	protected static enum Direction
	{
		...
		/**
		 * @param point a point in the grid. Its coordinates will be modified to
		 *              represent the next cell in the chosen direction.
		 */
