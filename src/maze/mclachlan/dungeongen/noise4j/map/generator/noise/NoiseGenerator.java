package mclachlan.dungeongen.noise4j.map.generator.noise;
...
/**
 * Divides grid into equal regions. Assigns semi-random value to each region
 * using a noise function. Interpolates the value according to neighbor regions'
 * values. Unless regions are too small, this usually results in a smooth map
 * with logical region transitions. During map generation, you usually trigger
 * the {@link #getRadius()} and {@link #getModifier()}, invoking generation
 * multiple times - each time with lower modifier. This allows to generate a map
 * with logical transitions, while keeping the map interesting thanks to further
 * iterations with lower radius.
 *
 * @author MJ
 */
public class NoiseGenerator extends AbstractGenerator implements CellConsumer
{
	 ...
	 */
	public static void generate(final Grid grid, final int radius,
	 ...
	 */
	private static void generate(final Grid grid, final int radius,
		final float modifier, final int seed)
	{
	...
	/**
	 * @return size of a single generation region. The bigger, the more smooth
	 * the map seems. Setting it to one effectively turns the map into
	 * semi-random noise with no smoothing whatsoever.
	 */
	...
	/**
	 * @param radius size of a single generation region. The bigger, the more
	 *               smooth the map seems. Setting it to one effectively turns
	 *               the map into semi-random noise with no smoothing
	 *               whatsoever.
	 */
	...
	/**
	 * @return relevance of the generation stage. Each grid cell will be
	 * increased with a semi-random value on scale from 0 to this modifier.
	 */
	...
	/**
	 * @param modifier relevance of the generation stage. Each grid cell will be
	 *                 increased with a semi-random value on scale from 0 to this
	 *                 modifier.
	 */
	 ...
	 */
	public void setAlgorithmProvider(
		final NoiseAlgorithmProvider algorithmProvider)
	{
	...
	{
		/**
		 * Semi-random function. Consumes two parameters, always returning the
		 * same result for the same set of numbers.
		 *
		 * @param generator its settings should be honored. Random seed is usually
		...
		/**
		 * Semi-random function. Consumes two parameters, always returning the
		 * same result for the same set of numbers. Noise is dependent on the
		 * neighbors, ensuring that drastic changes are rather rare.
		 *
		 * @param generator its settings should be honored. Random seed is usually
		 *                  used for the noise calculation.
		 * @param x         position on the X axis.
	...
	public static class DefaultNoiseAlgorithmProvider implements NoiseAlgorithmProvider
	{
		...
		@Override
		public float noise(final NoiseGenerator generator, final int x,
			final int y)
		{
		...
		@Override
		public float smoothNoise(final NoiseGenerator generator, final int x,
			final int y)
		{
			return // Corners:
		...
		@Override
		public float interpolate(final float start, final float end,
			final float factorial)
		{
