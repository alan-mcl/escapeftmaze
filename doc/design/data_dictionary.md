...
| CharacterSelection | `stat/CharacterSelection.java` | Ordered party selector: `methods` (inclusions) + `exclusions`; each step is a polymorphic `CharacterSelectionMethod` (player Who pick, lowest/highest/random modifier, modifier comparison, or custom `IMPL`). Modifier methods use effective `getModifier()` (includes race/gear), not base stats only. |
